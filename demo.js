...
/***/ function(module, exports, __webpack_require__) {
	...
	var _ProductCompare = __webpack_require__(2);

	var _ProductCompare2 = _interopRequireDefault(_ProductCompare);
	...
	(0, _jquery2.default)(function () {
	  ...
	  for (var i = 6; i > 0; i--) {
	    ...
	    (0, _jquery2.default)('.products').prepend(product({ value: i }));
	  }

	  var compare = new _ProductCompare2.default({
	    maxItems: 3
	  });

	  compare.on('beforeadd', function (id) {
	    console.log('compare before add');
	  });

	  compare.on('afteradd', function (id) {
	    console.log('compare after add');
	    console.log(compare.compareList.get(id).title + ' added to compare');
	  });

	  compare.on('beforeremove', function (id) {
	    console.log('compare before remove');
	  });

	  compare.on('afterremove', function (id) {
	    console.log('compare after remove');
	  });

	  compare.on('updated', function () {
	    console.log('compare updated');
	    (0, _jquery2.default)('.compare-title .num-items').text(compare.compareList.size);

	    if (compare.compareList.size > 0) {
	      (0, _jquery2.default)('[data-compare-widget]').show();
	    } else {
	      (0, _jquery2.default)('[data-compare-widget]').hide();
	    }
	  }, true);

	  (0, _jquery2.default)('.compare-remove-all').on('click', function () {
	    compare.removeAll();
	  });
	});
